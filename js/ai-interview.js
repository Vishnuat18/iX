...
// --- PHASE 3: SCORECARD ---
window.showScorecardPhase = async () => {
    ...
    try {
        ...
        if (user && user.uid) {
            const reportData = {
                ...
                metrics: {
                    communication: comm,
                    technical: 9, // Placeholder from hardcoded logic
                    confidence: 9.5, // Placeholder
                    grammar: Math.max(0, 10 - s.grammar),
                    behavior: Math.max(0, 10 - s.behavior)
                },
                mistakes: s.mistakes
            };

            // Use arrayUnion to store in the main document to leverage existing permissions
            await db.collection('users').doc(user.uid).update({
                interviewReports: firebase.firestore.FieldValue.arrayUnion(reportData)
            });
